-- Trigger de validação para custom_questions no teacher_feedback_settings
...
DROP TRIGGER IF EXISTS validate_custom_questions_trigger ON teacher_feedback_settings;

-- Criar trigger
CREATE TRIGGER validate_custom_questions_trigger
  BEFORE INSERT OR UPDATE ON teacher_feedback_settings
  FOR EACH ROW
  EXECUTE FUNCTION validate_custom_questions();

-- Comentário na tabela
COMMENT ON TRIGGER validate_custom_questions_trigger ON teacher_feedback_settings IS 
'Valida estrutura e conteúdo do campo custom_questions: máximo 10 perguntas, sem duplicatas, tipos válidos, campos obrigatórios presentes';