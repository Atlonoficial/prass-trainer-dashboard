-- FULL SETUP (retry with correct order)

-- 1) Enum for billing interval
DO $$ BEGIN
...
-- 7) Policies for plan_subscriptions
...
WITH CHECK (
  auth.uid() = student_user_id AND EXISTS (
    SELECT 1 FROM public.students s
...
-- 8) sync_student_membership function
...
-- 9) approve_subscription function (manual approval)
...
BEGIN
  ...
  END IF;
  IF sub.status <> 'pending' THEN RETURN 'noop'; END IF;
...
-- 10) trigger function and trigger to sync on changes
...
FOR EACH ROW EXECUTE FUNCTION public.trigger_sync_membership();

-- 11) get_user_entitlements function
