-- Corrigir a função book_appointment para aceitar student_id como parâmetro
...
begin
  ...
  -- Allow teacher to book for any of their students, or student to book for themselves
  if uid <> p_teacher_id and uid <> p_student_id then
    -- Check if caller is teacher and student belongs to them
    if not exists (
  ...
  -- Validate that student belongs to teacher
  if not exists (
    select 1 from public.students st
    where st.user_id = p_student_id and st.teacher_id = p_teacher_id
  ) then
  ...
  -- Insert appointment with student_id
  insert into public.appointments (
    ...
    extensions.uuid_generate_v4(), p_teacher_id, p_student_id, p_scheduled_time, step, appointment_status, p_type,
