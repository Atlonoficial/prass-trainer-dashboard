...
import { useCourseModules, CourseLesson } from '@/hooks/useCourseModules';
...
export default function EditLessonModal({ isOpen, onClose, lesson, moduleTitle, courseId }: EditLessonModalProps) {
  ...
  const { updateLesson } = useCourseModules(courseId);

  // Pré-popular os dados quando o lesson mudar
  useEffect(() => {
    if (lesson) {
      setLessonData({
        title: lesson.title || '',
        ...
        video_uploaded: lesson.video_uploaded || false,
        video_duration_minutes: lesson.video_duration_minutes || 0,
        content: lesson.content || '',
        is_free: lesson.is_free || false,
        is_published: lesson.is_published || false,
        enable_support_button: lesson.enable_support_button || false,
        release_mode: lesson.release_mode || 'immediate',
        release_after_days: lesson.release_after_days || undefined
      });
    }
  }, [lesson]);
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      let storagePath = lessonData.storage_path;

      // 1. Upload do vídeo se houver arquivo selecionado
      if (selectedFile) {
        // Se já existir um vídeo, idealmente deveríamos deletar o antigo, mas vamos apenas sobrescrever ou criar novo
        // Vamos criar um novo caminho para evitar problemas de cache
        const tempLessonId = crypto.randomUUID();
        const fileExt = selectedFile.name.split('.').pop();
        const filePath = `${courseId}/${lesson.module_id}/${tempLessonId}.${fileExt}`;
      ...
      toast({
        title: 'Erro',
        description: 'Erro ao atualizar a aula. Tente novamente.',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl p-0 flex flex-col">
        <DialogHeader className="px-5 pt-5 pb-3 pr-14 border-b border-border/40 flex-shrink-0">
          <DialogTitle>Editar Aula - {moduleTitle}</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {/* Status do vídeo atual */}
                {lessonData.video_uploaded && !selectedFile && (
                  <div className="mb-2 p-3 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center">
                      <FileVideo className="w-4 h-4 text-green-500" />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium text-green-700 dark:text-green-400">Vídeo enviado</p>
                      <p className="text-xs text-muted-foreground">O vídeo já está salvo no servidor</p>
                    </div>
                  </div>
                )}
                ...
                {!selectedFile ? (
                  <div className="border-2 border-dashed border-border rounded-lg p-6 flex flex-col items-center justify-center text-center hover:bg-muted/50 transition-colors cursor-pointer relative">
                    ...
                    <p className="text-sm font-medium">
                      {lessonData.video_uploaded ? 'Substituir vídeo atual' : 'Clique para enviar o vídeo'}
                    </p>
